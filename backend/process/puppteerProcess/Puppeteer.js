...
async function getDirPath(domain, title) {
    ...
    let dir = path.join(__dirname, dirPath + '/' + title.replace(/[?:\|，。"']/g, '').split(' ')[0] + '.json');
    stat = fs.existsSync(dir);
    console.log(`${stat}: ${dir}`);
    if (stat) {
        dir = path.join(__dirname, dirPath + '/' + title.replace(/[?:\|，。"']/g, '') + '.json')
    }
    return dir;
}
...
// test
    ...
    for (let j = 0, length = spiderNode.length; j < length; j++) {
        ...
        node.cate = cateIndex;
        if (maxRate < 0.6 || node.content.length < 10 || node.item.linkElementCount === 0 || node.item.imageElementCount === 0) {
            continue;
        }
        if (cateIndex === 0) {
